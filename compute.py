...
if consensus_path.exists():
    ...
    # BASE = Sleeper ADP (Alex's call 2026-08-31: our leagues live on Sleeper,
    # so its drafters are the population our leaguemates come from). FFC is
    # still carried as `fadp` for comparison, and its dispersion as `asd`.
    #
    # The draft-logic layer stays on top: where the human analysts sit far from
    # the market, that gap is news the market has not priced yet, so shade
    # halfway toward them. Vegas is excluded -- prop lines lag hardest.
    HUMAN = ("joel", "flock", "fb", "ffa")
    SHADE_AT = 15          # picks of analyst-vs-market gap before news shading
    shaded = 0
    for e in players_out:
        base = e.get("adp") if e.get("adp") is not None else e.get("fadp")
        if base is None:
            e["madp"] = None
            continue
