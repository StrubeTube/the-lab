...
(function () {
  LAB.tradeCore = function (players, board, L, TR, allTrades) {
    ...
    const pickVal = (season, round, b) => {
      ...
      const base = b === 'true' ? V(midPick(Math.min(16, +round)))
  ...
  LAB.tradeFinder = function (C, L, opts) {
    ...
    const spares = me.players.map(pid => C.byId[pid])
      ...
      .sort((x, y) => y.s - x.s);
    const pv = r => C.pickVal(L.season, r, b);
      ...
      for (const sp of spares.slice(0, 4)) {
        ...
        for (const r of rr) {
          push({
            ...
            rung: r.label, chips,
            myGain: pv(r.pick.round), theirGain: gain,
        ...
        if (o.maxAssets >= 3) {
          ...
          if (sweet && up && gain >= 3) {
            push({
              ...
              rung: 'PACKAGE', chips,
              myGain: pv(up.round) - pv(sweet.round), theirGain: gain + pv(sweet.round) - 0,
        ...
        if (gain2 >= 8) {
          ...
          if (up) {
            push({
              ...
              rung: 'PAIR', chips,
              myGain: pv(up.round), theirGain: gain2,
        ...
        for (const ts of theirSpares) {
          ...
          for (const r of rr) {
            ...
            push({
              ...
              rung: r.label, chips,
              myGain: myGain - pv(r.pick.round) / 3, theirGain: pv(r.pick.round),
      ...
      if (myOwned < 16 && extra > 0 && pOpen.length >= 2) {
        for (const a of myOpen) {
          ...
          for (let i = 0; i < later.length - 1; i++) {
            const net = pv(later[i].round) + pv(later[i + 1].round) - pv(a.round);
